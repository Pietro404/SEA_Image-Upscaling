...
void cpu_nn_v1(
    ...
    for (int y = 0; y < new_height; y++) {
        ...
        for (int x = 0; x < new_width; x++) {
            ...
            /*1. Il mistero del ciclo channels: Perché raddoppia il tempo?
            In teoria, un ciclo for (int c = 0; c < 3; c++) dovrebbe essere identico a tre assegnazioni manuali.
            In pratica, però, accadono due cose a livello di compilatore:
            Mancata Unrolling (Srotolamento): Se il compilatore non è sicuro che channels sia una costante piccola (come 3),
            non può "srotolare" il ciclo. Questo introduce un overhead di controllo (incremento di c, confronto c < channels,
            salto condizionato) per ogni singolo pixel. Moltiplicato per milioni di pixel, il costo è enorme.
            Aliasing e Ottimizzazione: Con le assegnazioni manuali +0, +1, +2,
            il compilatore vede tre accessi a memoria con offset costanti e può generare una singola istruzione di caricamento/scrittura a 32 bit (tralasciando l'ultimo byte).
            Con un ciclo variabile, è costretto a generare istruzioni di lettura/scrittura da 1 byte singolo, che sono molto più lente perché non sfruttano la larghezza del bus dati della CPU.*/
...
void cpu_nn_sse2(
    ...
    // ma per chiarezza la lasciamo scalare (è eseguita una sola volta).
    for (int x = 0; x < new_width; x++) {
    ...
    for (int y = 0; y < new_height; y++) {
        ...
        // Controlliamo se questa riga di output inizia su un indirizzo allineato
        //bool row_aligned = is_aligned(dst_row, 16);
        ...
        for (; x <= new_width - 16; x += 16) {
            ...
            // Su CPU moderne questo è veloce, su vecchie potrebbe essere meglio l'accesso scalare diretto.
            ...
            // 3. STITCHING & STORE (Scrittura in memoria)
            ...
            // 4. SCRITTURA CON _mm_store_si128 (Se allineato)
            //if (row_aligned) {
            ...
            //} else {
            //    // SICURO: Scrittura non allineata (leggermente più lenta su vecchie CPU)
